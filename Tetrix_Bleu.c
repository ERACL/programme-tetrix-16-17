...
task main()
{
	// Initialisation des encoders des roues
	nMotorEncoder[motorA]=0;
	nMotorEncoder[motorB]=0;
	nMotorEncoder[pince]=0;

	// Initialisation des variables pour la librarie de mouvements
	//rayon = 46.3;
	rayon = 25.1;
	matrix_or_tetrix=2;
	offset_motor=15;
	...
	obstacle = 1;
	// Distance milieu roue droite / milieu roue gauche
	entraxe = 190;
	start=0;

	// Projet d'une fonction pour savoir exactement ou est-ce que l'on est sur le terrain. Jamais implemente.
	//startTask(position);


	// Fonctions specifiques a la pince du robot.
	// L'interet de faire des task est qu'ils tournent en arriere-plan. 
	startTask(rentre_verrinT);

	// Initialisation du bouton de demarrage et d'arret d'urgence
	startTask(redButton);
	start=0;
	while(start == 0)
		wait1Msec(10);

	// Des que le match commence, on lance la tache pour la funny action. cf code de la funny action.
	startTask(funnyAction);

	// Autres variables pour la librairie de deplacements. Cf doc en ligne.
	...
	// Desactivation de la detection d'obstacles
	obstacle = 0;
	...
	/*
	// Ces fonctions ne fonctionne que sur NXT. Affiche a l'ecran des valeurs. Cf doc RobotC.
	...
	*/
	...
	// Passage de pointeurs dans les fonctions de deplacement avec le "&".
	recule_nulle(90, &dist, &done, &obs, 1000);
	obstacle = 1;
	wait1Msec(100);
	avance_nulle_anyway(120, &dist, &done, &obs, 3000);
	wait1Msec(100);
	turnRightDeg(180, &angle, &done);
	wait1Msec(100);
	...
	// Prise du cylindre dans la tour, on parcourt un rectangle
	wait1Msec(300);
	...
	wait1Msec(100);
	avance_nulle_anyway(125, &dist, &done, &obs);
	wait1Msec(100);
	turnLeftDeg_safe(165);
	obstacle = 0;
	wait1Msec(100);
	// Prise du cylindre
	...
	for(int i = 0; i < 2; i++)
	{
		...
		obstacle = 0;
		wait1Msec(100);
		turnRightDeg_safe(100);
		wait1Msec(200);
	}

	recule_nulle_anyway(65, &dist, &done, &obs);
	turnRightDeg_safe(50);
	obstacle = 1;
	avance_nulle_anyway(145, &dist, &done, &obs, 0, 13);
	obstacle = 0;
	turnLeftDeg_safe(90);
	wait1Msec(100);
	turnLeftDeg_safe(90);
	wait1Msec(100);
	avance_nulle_anyway(50, &dist, &done, &obs, 0, 13);
	wait1Msec(100);
	obstacle = 1;
	recule_nulle_anyway(30, &dist, &done, &obs, 0, 13);
	//turnLeftDeg_safe(90);
	wait1Msec(100);

	//obstacle = 1;
	//avance_nulle_anyway(350, &dist, &done, &obs, 0, 18);



	// On s'assure que c'est bien la tache funny action qui arretera le robot ! Sinon le programme s'arrete prematurement !
	wait1Msec(90000);
}
